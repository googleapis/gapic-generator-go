...
func (g *generator) noRequestStreamCall(servName string, s *descriptorpb.ServiceDescriptorProto, m *descriptorpb.MethodDescriptorProto) error {
	...
	p(`    c.logger.DebugContext(ctx, "api streaming client response", "serviceName", serviceName, "rpcName", %q)`, m.GetName())
	p("    return err")
	p("  }, opts...)")
	p("  if err != nil {")
	p("    return nil, err")
...
func (g *generator) serverStreamCall(servName string, s *descriptorpb.ServiceDescriptorProto, m *descriptorpb.MethodDescriptorProto) error {
	...
	p("  var err error")
	p(`  c.logger.DebugContext(ctx, "api streaming client request", "serviceName", serviceName, "rpcName", %q)`, m.GetName())
	p("  resp, err = c.%s.%s(ctx, req, settings.GRPC...)", grpcClientField(servName), m.GetName())
	p(`  c.logger.DebugContext(ctx, "api streaming client response", "serviceName", serviceName, "rpcName", %q)`, m.GetName())
	p("  return err")
	p("}, opts...)")
	p("if err != nil {")
